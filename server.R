...
server <- function(input, output, session) {
  ...
  comparisons_dm <- reactive({
    comparisons <- gsub("_p.adj", "", colnames(SummarizedExperiment::rowData(dep_dm()))[grep(
      "p.adj",
  ...
  volcano_input_selected_dm <- reactive({
    if (!is.null(input$volcano_cntrst_dm)) {
      ...
      ## convert contrast to x and padj to y
      diff_proteins <- grep(
        paste(input$volcano_cntrst_dm, "_log2", sep = ""),
        colnames(proteins_selected)
      )
      if (input$p_adj_dm == "FALSE") {
        padj_proteins <- grep(
          paste(input$volcano_cntrst_dm, "_p.val", sep = ""),
          colnames(proteins_selected)
        )
      } else {
        padj_proteins <- grep(
          paste(input$volcano_cntrst_dm, "_p.adj", sep = ""),
          colnames(proteins_selected)
        )
      }
  ...
  go_input_dm <- eventReactive(input$go_analysis_dm, {
    ...
    if (!is.null(input$contrast_dm)) {
      ...
      plot_go <- plot_enrichment(go_results,
        number = 5, alpha = 0.05, contrasts = input$contrast_dm,
  ...
  pathway_input_dm <- eventReactive(input$pathway_analysis_dm, {
    withProgress(
      message = "Pathway enrichment is in progress",
      detail = "Please wait for a while",
      value = 0,
      {
        for (i in 1:15) {
    ...
    plot_pathway <- plot_enrichment(pathway_results,
      number = 5, alpha = 0.05, contrasts = input$contrast_dm_1,
  ...
  output$downloadData_dm <- downloadHandler(
    filename = function() {
      paste(input$dataset_dm, ".csv", sep = "")
    }, ## use = instead of <-
    content = function(file) {
  ...
  output$downloadCluster_dm <- downloadHandler(
    filename = function() {
      paste("Cluster_info_", input$cluster_number_dm, ".csv", sep = "")
    }, ## use = instead of <-
    content = function(file) {
  ...
  output$downloadGO_dm <- downloadHandler(
    filename = function() {
      paste("GO_enrichment_", input$go_database_dm, ".csv", sep = "")
    }, ## use = instead of <-
    content = function(file) {
  ...
  output$downloadPA_dm <- downloadHandler(
    filename = function() {
      paste("Pathway_enrichment_", input$pathway_database_dm, ".csv", sep = "")
    },
    ## use = instead of <-
    content = function(file) {
